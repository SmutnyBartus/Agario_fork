...
typedef enum { WAITING_FOR_HOST, CONNECTED } GameStage;
...
int main(void) {
    ...
    Rectangle text_box = {screenWidth / 2.0f - 275, 180, 550, 50};

    GameStage game_stage = WAITING_FOR_HOST;
    while (!WindowShouldClose()) {
        ...
        case WAITING_FOR_HOST: {
            ...
            if (IsKeyPressed(KEY_ENTER)) {
                ...
                if ((numbytes = sendto(udp_socket, buf, 3, 0, p->ai_addr,
                                       p->ai_addrlen)) == -1) {
                    perror("ERROR: sendto()");
                    exit(1);
                }
                ...
                numbytes =
                    recvfrom(udp_socket, &buf2, sizeof(buf2), 0, &addr, &len);
                printf(
                    "INFO: received %d bytes, message type is %d, len is %d\n",
                    numbytes, buf2.type, buf2.length);

                fcntl(udp_socket, F_SETFL, O_NONBLOCK);
            }
            while (key > 0) {
                if ((key >= 32 && key <= 125) && (lettercount < IP_LEN)) {

                    host[lettercount] = (char)key;
                    host[lettercount + 1] = '\0';
                    lettercount++;
                }
            ...
            DrawText(host, (int)text_box.x + 5, (int)text_box.y + 8, 40,
                     MAROON);
            DrawText(TextFormat("INPUT CHARS: %i/%i", lettercount, IP_LEN), 315,
                     250, 20, DARKGRAY);
            EndDrawing();
            break;
        }
        case CONNECTED: {
            ...
            numbytes = sendto(udp_socket, &angle_buf, sizeof(angle_buf), 0,
                              &addr, len);
            if (numbytes == -1) {
            ...
            // Vector2 center = {(float)screenWidth / 2, (float)screenHeight /
            // 2};
            //
            ...
            */
            ...
            for (int i = 0; i < game_state.n_players; i++) {
                Vector2 pos = {game_state.players[i].pos.x,
            ...
            for (int i = 0; i < game_state.n_fruits; i++) {
                Vector2 pos = {game_state.fruits[i].pos.x,
                               game_state.fruits[i].pos.y};
